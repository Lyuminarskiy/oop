...
Сигнатура | Возвр. тип | Описание
--------- |:----------:| --------
`copy()` | `Citizen` | Возвращает копию гражданина.
...
Сигнатура | Возвр. тип | Описание
--------- |:----------:| --------
`copy()` | `Worker` | Возвращает копию работника.
...
1. Создайте тип магазина `Store` и вспомогательный тип `StoreReader`,
предназначенный для вывода информации о продуктах для продажи.

2. Создайте тип продукта `Product` и его разновидности:
...
3. Создайте тип `Expiring`, представляющий что-либо заканчивающееся.

5. Для каждого из типов определите, чем он является:

    - обычным классом,
    - абстрактным классом,
    - интерфейсом.

5. Определите и установите отношения наследования между типами.

6. Определите модификаторы доступа членов классов.

7. Продемонстрируйте работу с типами.
...
Имя | Тип | Доступ | Описание
:--:|:---:|:------:| --------
`products` | `Product[]` | `Чтение` | Продукты для продажи.
...
Сигнатура | Описание
--------- | --------
`Store()` | Создаёт пустой магазин.
`Store(Product[])` | Создаёт магазин с указанными продуктами для продажи.
...
Сигнатура | Возвр. тип | Описание
--------- |:----------:| --------
`static` `getExpired(Store)` | `Product[]` | Возвращает закончившиеся продукты.
`static` `getUnexpired(Store)` | `Product[]` | Возвращает незакончившиеся продукты.
...
Имя | Тип | Доступ | Описание
:--:|:---:|:------:| --------
`name` | `string` | `Чтение` | Название продукта.
...
Сигнатура | Описание
--------- | --------
`Product(string)` | Создаёт продукт с указанным названием.
...
Сигнатура | Возвр. тип | Описание
--------- |:----------:| --------
`toString()` | `string` | Возвращает строку* с информацией о продукте.

Примечания:

_* Примеры строк для заканчивающихся продуктов:_

```
Продукт1 (не закончился)
Продукт2 (закончился)
```

_Пример строки для остальных продуктов:_
...
Сигнатура | Возвр. тип | Описание
--------- |:----------:| --------
`isExpired()` | `bool` | Возвращает признак того, что текущий экземпляр закончился.

## 2.3. Обобщённые классы и методы

Задачи:

1. Создайте обобщённый тип двусвязного списка `List<T>`,
...
`Item(T value, Item<T> prev, Item<T> next)` | Создаёт элемент и сохраняет в нём значение `value`, а также ссылки на предыдущий `prev` и последующий `next` элементы списка.
...
`tail` | `Item<T>` | - | Ссылка на последний элемент списка.
...
Сигнатура | Возвр. тип | Описание
--------- |:----------:| --------
`append(T)`| - | Добавляет новый элемент с указанным значением в конец списка.
...
`static` `equals(List<T> first, List<T> second)` | `bool` | Возвращает результат поэлементного сравнения списка `first` со списком `second`.
`getItem(int)` | `T` | Возвращает значение, хранящееся в элементе списка с указанным индексом.
`indexOf(T value, int index)` | `int` | Возвращает индекс первого найденного вхождения значения `value` в пределах всего списка, начиная с индекса `index`.
...
`lastIndexOf(T value, int index)` | `int` | Возвращает индекс последнего найденного вхождения значения `value` в пределах всего списка, начиная с индекса `index`.
`prepend(T)` | - | Добавляет новый элемент с указанным значением в начало списка.
...
`remove(int start, int end)` | - | Удаляет элементы списка, начиная с индекса `start` (включительно) и заканчивая на индексе `end` (не включительно).
`reverse()` | - | Обращает порядок элементов в списке.

::: warning Обратите внимание!
При удалении списка должны удаляться и его элементы.
Если вы используете язык с ручным управлением памятью (например, `C++`),
то создайте **деструктор класса**.
:::